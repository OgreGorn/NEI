...
Формальные правила грамматики для конструкции `iteration-statement` извлечены из приложения A.5 официального черновика стандарта C++17 (N4659). Примеры синтаксических конструкций проверены на этапе компиляции в файле check7.cpp.

### Ход выполнения и результаты тестирования:

Создали файл `check7.cpp`, содержащий реализации всех четырёх типов циклов, описанных в грамматике стандарта, и успешно скомпилировали его командой `g++ check7.cpp -o check7.exe`.
![Успешная компиляция и выполнение check7.exe в Far](screen/p10.png)

### Формат ответа по стандарту C++:
...
1. **Первый вид** — цикл **с предусловием (while)**, который соответствует строке правила:
   ...
   - **Описание на естественном языке:** Ключевое слово «while» в круглых скобках условие «condition», далее инструкция «statement». Тело цикла выполняется до тех пор, пока проверяемое условие истинно.
   - **Пример программы:**
     ```cpp
     int x = 1;
     while (x < 1000) {
...
2. **Второй вид** — цикл **с постусловием (do-while)**, который соответствует строке правила:
   ...
   - **Описание на естественном языке:** Ключевое слово «do» далее инструкция «statement», далее ключевое слово «while» в круглых скобках выражение-условие «expression» и обязательный символ точки с запятой «;». Гарантирует как минимум однократное выполнение тела цикла перед проверкой условия.
   - **Пример программы:**
     ```cpp
     int y = 1;
     do {
...
3. **Третий вид** — традиционный **итерационный цикл (for)**, который соответствует строке правила:
   `for ( init-statement condition_opt ; expression_opt ) statement`
   - **Описание на естественном языке:** Ключевое слово «for» в круглых скобках начальная инструкция инициализации «init-statement», необязательное условие «condition_opt», разделитель точка с запятой «;», необязательное выражение шага цикла «expression_opt», далее инструкция тела цикла «statement».
   - **Пример программы:**
     ```cpp
     for (int i = 0; i < 3; i++) {
         int temp = i;
...
4. **Четвёртый вид** — диапазонный **цикл по коллекции (range-based for)**, который соответствует строке правила:
   `for ( for-range-declaration : for-range-initializer ) statement`
   - **Описание на естественном языке:** Ключевое слово «for» в круглых скобках объявление переменной текущего элемента «for-range-declaration», разделитель двоеточие «:», итерируемый объект-коллекция «for-range-initializer», далее инструкция тела цикла «statement».
   - **Пример программы:**
     ```cpp
     int arr[] = {1, 2, 3};
     for (int n : arr) {
...
Синтаксическая структура выражения `a = -x + y * (z - 1);` из проверочного файла check8.cpp проанализирована и представлена в виде иерархического дерева.

### Ход выполнения и результаты тестирования:

Создали файл `check8.cpp`, содержащий целевую программу, и успешно провели синтаксический контроль компилятором с помощью команды `g++ check8.cpp -o check8.exe`.
![Успешная компиляция check8.cpp в Far Manager](screen/p11.png)

### Структура Абстрактного Синтаксического Дерева (AST):

При построении дерева узлами (родительскими элементами) выступают операторы, расположенные в порядке возрастания их приоритета выполнения, а листьями — идентификаторы переменных и константные литералы.
...
```text
          ...
          Ident: a     [ + ] (Сложение)
              ...
              [ - ] (Унарный) [ * ] (Умножение)
                |            /     \
             Ident: x    Ident: y   [ - ] (Вычитание)
                                   /     \
                               Ident: z   Const: 1
```

### Порядок синтаксического разбора дерева парсером:
1. Вычисляется узел самого высокого приоритета в круглых скобках: `(z - 1)`. Оператор вычитания `[-]` принимает операнды `z` и `1`.
2. Вычисляется оператор унарного минуса `[-]` для переменной `x`.
3. Вычисляется оператор умножения `[*]`. Его левым операндом становится идентификатор `y`, а правым — результат поддерева вычитания `(z - 1)`.
4. Вычисляется оператор бинарного сложения `[+]`. Он складывает результат унарного минуса `-x` и результат поддерева умножения.
5. Вычисляется корневой оператор присваивания `[=]`, который записывает итоговое значение всего вычисленного правого поддерева в переменную `a`.
